...
function writeClanMembersToCSV(clanData) {
  ...
  // Add each clan member to the CSV content
  clanMembers.forEach((member) => {
    csvContent += `${member.tag},${member.name},${member.townHallLevel},${member.expLevel},${member.trophies},${member.role}\n`;
  });

  console.log("writeClanMembersToCSV is running...");
  ...
  fs.appendFile(
    ...
    (err) => {
      if (err) {
        ...
        console.log("Clan members data saved to clan_members.csv");
      }
    }
  );
}
